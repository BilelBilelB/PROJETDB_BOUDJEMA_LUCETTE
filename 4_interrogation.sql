...
SELECT projet_titre, budget_reel FROM projet WHERE budget_reel > 600000 ORDER BY budget_reel DESC; 

#Liste des personnes qui ont un rôle de type "Scénariste" ou "Réalisatrice"
SELECT nom_personne, prenom_personne, role_personne FROM personne WHERE role_personne IN ('Scénariste', 'Réalisatrice'); 
...
# Téléphones uniques des électricien
SELECT DISTINCT telephone_personne FROM personne WHERE role_personne = 'Électricien'; 
...
# Projets avec un total des contrats supérieur à 40 000 €
SELECT p.id_projet, p.projet_titre, SUM(c.montant_contrat) AS total_contrats FROM projet p JOIN Participation pa ON p.id_projet = pa.id_projet JOIN contrat c ON pa.id_contrat = c.id_contrat GROUP BY p.id_projet, p.projet_titre HAVING SUM(c.montant_contrat) > 40000; 
...
# Projets ayant au moins un contrat supérieur à 5 500 €
SELECT projet_titre FROM projet p WHERE EXISTS ( SELECT 1 FROM Participation pa JOIN contrat c ON pa.id_contrat = c.id_contrat WHERE pa.id_projet = p.id_projet AND c.montant_contrat > 5500 ); 
